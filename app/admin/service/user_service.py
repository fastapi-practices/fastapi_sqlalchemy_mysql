...
from app.admin.crud.crud_user import UserDao
from database.db_mysql import async_db_session
from app.admin.model import User
from app.admin.schema.user import CreateUser, ResetPassword, UpdateUser, Auth2, Avatar
...
class UserService:
    ...
    async def login_swagger(*, form_data: OAuth2PasswordRequestForm) -> tuple[str, User]:
        async with async_db_session() as db:
            ...
            await UserDao.update_user_login_time(db, user.username, login_time=timezone.now())
    ...
    async def update_status(*, current_user: User, pk: int) -> int:
        async with async_db_session.begin() as db:
            await superuser_verify(current_user)
            if not await UserDao.get(db, pk=pk):
                raise errors.NotFoundError(msg='用户不存在')
            else:
                if pk == current_user.id:
